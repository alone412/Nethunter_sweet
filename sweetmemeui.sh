...
CODENAME="sweet"
KERNEL_NAME="Sleepy-Hunter"

# Kernel build release tag
KRNL_REL_TAG="MIUI"

DEFCONFIG="sweet_full_defconfig"
...
#		git clone --depth=1 https://github.com/kdrag0n/proton-clang.git "$HOME"/clang
	...
	export KBUILD_COMPILER_STRING=$(/usr/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
fi
...
# memeui stuff
#git revert 6bb9dba1e29cf515e0c9156a6f830fb36517f436 --no-edit
...
tg_post_msg "Successful triggered Compiling kernel for sweet memeui" "$CHATID"
